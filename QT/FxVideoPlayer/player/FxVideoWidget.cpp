...
FxVideoWidget::FxVideoWidget(QWidget *parent)
    : QWidget{parent}
{
    // 适用调色板设置背景色
 ...
 * @param image
 */
void FxVideoWidget::updateImage(const QImage& image)
{
 ...
 * @param pixmap
 */
void FxVideoWidget::updatePixmap(const QPixmap &pixmap)
{
    m_mutex.lock();
    m_pixmap = pixmap;
    m_mutex.unlock();
 ...
 * @param event
 */
void FxVideoWidget::paintEvent(QPaintEvent *event)
{
    if(!m_pixmap.isNull())
    {
